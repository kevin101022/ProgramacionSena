/**
 * Calendario de Instructor - Reporte
 */
class CalendarioInstructorManager {
    constructor() {
        this.calendar = null;
        this.instructores = [];
    ...
    bindEvents() {
        ...
        if (instructorSearch) {
            instructorSearch.addEventListener('focus', () => this.renderInstructorDropdown(instructorSearch.value));
            instructorSearch.addEventListener('input', (e) => this.renderInstructorDropdown(e.target.value));
            instructorSearch.addEventListener('click', (e) => {
                e.stopPropagation();
                this.renderInstructorDropdown(e.target.value);
            });
        }

        document.addEventListener('click', (e) => {
            const instructorDropdown = document.getElementById('instructorDropdown');
            if (instructorDropdown && instructorSearch && !instructorSearch.contains(e.target) && !instructorDropdown.contains(e.target)) {
    ...
    renderInstructorDropdown(filter = '') {
        const instructorDropdown = document.getElementById('instructorDropdown');
        if (!instructorDropdown) return;

        instructorDropdown.innerHTML = '';
        const searchTerm = filter.toLowerCase().trim();

        const filtered = this.instructores.filter(i => {
            const doc = String(i.numero_documento || i.inst_id || '').toLowerCase();
            const nombres = (i.inst_nombres || '').toLowerCase();
            const apellidos = (i.inst_apellidos || '').toLowerCase();
            return doc.includes(searchTerm) || nombres.includes(searchTerm) || apellidos.includes(searchTerm);
        });

        if (filtered.length === 0) {
            instructorDropdown.innerHTML = '<div class="p-4 text-center text-sm text-gray-500">No se encontraron instructores</div>';
        } else {
            filtered.forEach(i => {
                const item = document.createElement('div');
                item.className = 'custom-dropdown-item';
                item.innerHTML = `
                    <div style="font-weight: 700; font-size: 0.9rem;">${i.inst_nombres || ''} ${i.inst_apellidos || ''}</div>
                    <div style="font-size: 0.75rem; color: #6b7280;">Doc: ${i.numero_documento || i.inst_id || 'N/A'}</div>
                `;
                item.onclick = (e) => {
                    e.stopPropagation();
                    this.selectInstructor(i);
                };
    ...
    async selectInstructor(i) {
        ...
        await this.loadAsignacionesInstructor(i.numero_documento || i.inst_id);
        await this.loadAllDetalles();
        this.initCalendar();
    }

    async loadAsignacionesInstructor(instId) {
        try {
    ...
    initCalendar() {
        ...
        const events = this.allDetalles.map((d) => {
            ...
            return {
                ...
                title: `${horaIni}-${horaFin} | Ficha ${asig.ficha_fich_id || asig.fich_id} — ${asig.comp_nombre_corto || 'Comp.'} (Amb. ${asig.ambiente_amb_id || ''})`,
    ...
    async downloadPDF() {
        if (!this.selectedInstructor || !this.calendar) {
            alert('Primero seleccione un instructor');
            return;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    window.calendarioInstructorManager = new CalendarioInstructorManager();
});
